...
from hotr.CLIP import clip

object_class = ['N/A', 'person', 'bicycle', 'car', 'motorcycle', 'airplane', 'bus',
...
class HOTR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, _ = self.detr.transformer(self.detr.input_proj(src), mask, self.detr.query_embed.weight, None, pos[-1])
        ...
        for bs in outputs_class[-1]:
            prob = F.softmax(bs, -1)
            scores, labels = prob[..., :-1].max(-1)
            scores, indices = torch.sort(scores)
            labels = [labels[indice] for indice in indices]
            objects_name = [object_class[labels[inx].to(dtype=torch.long, device='cuda')] for inx in range(-16,0)]
            text = clip.tokenize(objects_name).to('cuda')
            with torch.no_grad():
              text_features = self.CLIP_model.encode_text(text)
            semantic_queries = self.linear(text_features.float())
            all_semantic_queries.append(semantic_queries)
        ...
        interaction_hs = self.interaction_transformer(self.detr.input_proj(src), mask, self.query_embed.weight, all_semantic_queries, pos[-1])[0] # interaction representations
